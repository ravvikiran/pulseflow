...
function SectorRow({ sector, rank, timeframe }: {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 hover:bg-accent/20 transition-colors border-b border-border/50">
      <span className="text-2xs text-muted-foreground w-5 tabular-nums shrink-0">{rank}</span>
      <div className="flex-1 min-w-0">
        <div className="text-xs font-semibold text-foreground">{sector.sector}</div>
        <div className="flex items-center gap-2 mt-0.5">
          <div className="flex items-center gap-1">
            <span className="text-3xs text-muted-foreground">Mom</span>
          ...
          <div className="flex items-center gap-1">
            <span className="text-3xs text-muted-foreground">Str</span>
            <div className="w-10 h-1 bg-surface-3 rounded-full overflow-hidden">
              <div className="h-full bg-bull rounded-full" style={{ width: `${sector.strengthScore}%` }} />
            </div>
          </div>
          <div className="flex items-center gap-1">
            <span className="text-3xs text-muted-foreground">Vol</span>
            <div className="w-10 h-1 bg-surface-3 rounded-full overflow-hidden">
              <div className="h-full bg-[var(--color-warning)] rounded-full" style={{ width: `${sector.volumeScore}%` }} />
            </div>
          </div>
        </div>
      </div>
      <div className="flex items-center gap-3 shrink-0">
        ...
        <span className={cn("score-badge text-2xs w-8",
          score >= 70 ? "bg-bull/15 text-bull" : score >= 40 ? "bg-[var(--color-warning)]/15 text-warning" : "bg-bear/15 text-bear"
    ...
    <div className="p-4 md:p-6 space-y-5 animate-[fade-up_0.3s_ease-out]">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">NSE sector rankings, momentum, and inflow/outflow analysis</p>
          </div>
        </div>
      ...
      <div className="pf-card p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="text-3xs ml-auto">Click to select</Badge>
        </div>
        {isLoading ? (
      ...
      {selectedSector && sectorDetail && (
        <div className="pf-card p-4 space-y-3 border-l-2 border-l-primary">
          <div className="flex items-center justify-between">
            ...
            <Button variant="ghost" size="sm" className="h-6 text-2xs" onClick={() => setSelectedSector(null)}>✕ Close</Button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <div className="text-2xs text-muted-foreground uppercase tracking-wider">Top Stocks in Sector</div>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={inflowData} layout="vertical" margin={{ left: 60, right: 20 }}>
                ...
                <YAxis type="category" dataKey="name" tick={{ fontSize: 9, fill: "var(--color-muted-foreground)" }} width={55} />
                <Tooltip
                ...
                <Bar dataKey="inflow" radius={[0, 3, 3, 0]}>
                  {inflowData.map((entry, index) => (
                    <Cell key={index} fill={entry.inflow >= 0 ? "var(--color-bull)" : "var(--color-bear)"} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
